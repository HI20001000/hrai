...
const jobDictionaryOptions = computed(() => {
  const options = sortedDictionaryEntries.value.map((jobKey) => ({
    value: jobKey,
    label: jobDictionary.value?.[jobKey]?.title || jobKey,
  }))
  const currentJobKey = String(jobPostForm.value.jobKey || '').trim()
  if (currentJobKey && !jobDictionary.value?.[currentJobKey]) {
    options.unshift({
      value: currentJobKey,
      label: selectedJobPost.value?.jobSnapshot?.title || selectedJobPost.value?.title || currentJobKey,
    })
  }
  return options
})
