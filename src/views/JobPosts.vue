...
const selectedDictionaryTitle = computed(() => {
  if (isCreatingJobPost.value) return jobPostForm.value.jobKey
  return selectedJobPost.value?.jobSnapshot?.title || selectedJobPost.value?.jobKey || ''
})
...
<template>
  <section class="job-post-page">
        ...
        <div class="form-grid">
          ...
          <label class="field">
            ...
            <select v-model="jobPostForm.jobKey" :disabled="!isCreatingJobPost">
              <option v-for="jobTitle in sortedDictionaryEntries" :key="jobTitle" :value="jobTitle">
                {{ jobTitle }}
              </option>
            </select>
          </label>
    ...
    <JobDictionaryPanel :selected-title="selectedDictionaryTitle" @updated="handleDictionaryUpdated" />
  </section>
</template>
